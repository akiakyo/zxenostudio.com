...
export async function send(session: Session, body: Record<string, any>) {
  ...
  /* Sent in the poster's name ("Aquio Fulgencio · ZXENO Studio"), replies go
     to them, and each teammate is greeted by name. A brand-only sender is one
     of the signals Gmail uses to file mail under Promotions instead of Primary. */
  ...
  const author = announcement.author ? senderName(announcement.author) : "";
  const from = author
    ? `${author} · ZXENO Studio <${FROM.replace(/^.*<|>$/g, "")}>`
    : FROM;
