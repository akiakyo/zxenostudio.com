...
import { ErrorNote, Panel, Progress, StatusBadge } from '../../ui/ui';
...
export function DashboardSummary(){
 ...
 return <div className="dash-grid">
  ...
  <Panel title="Studio today" action={<Link className="panel-link" to="/team">Team directory</Link>}><div className="hq-stack hq-pad"><div className="row-buttons">{data.people.map(p=><span key={p.workStatus}>{p.count} {label(p.workStatus).toLowerCase()}</span>)}</div><h3>In use now</h3>{data.rooms.map((r,i)=><div key={i}><strong>{r.location}</strong><span className="cell-sub">{r.title} · until {r.endTime}</span></div>)}{!data.rooms.length&&<p className="muted">No room bookings in progress.</p>}</div></Panel>
 </div>;
}
