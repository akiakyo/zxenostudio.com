...
export function WorkspaceTools(){
 ...
 return <>
  <div className="hq-tools" aria-label="Workspace tools"><Button size="sm" icon={Search} onClick={()=>setOpen(true)}>Search <kbd>Ctrl K</kbd></Button><Button size="sm" icon={Bell} onClick={()=>{setBell(true);notifications.reload();}}>Notifications{notifications.data?.some(n=>!n.read)?' •':''}</Button><Menu label="Create new" items={[{label:'New project',icon:Plus,onSelect:()=>project.openNew()},{label:'New task',icon:Plus,onSelect:()=>task.openNew()},{label:'New event',icon:Plus,onSelect:()=>event.openNew(eventConfig.defaults)},{label:'New invoice',icon:Plus,onSelect:()=>invoice.openNew()}]}/></div>
  ...
  <Modal open={bell} title="Notifications" onClose={()=>setBell(false)} footer={<Button onClick={async()=>{if(await run(()=>post('notifications',{ids:(notifications.data??[]).filter(n=>!n.read).map(n=>n.id)}),'Notifications marked read'))notifications.reload();}}>Mark all read</Button>}>
